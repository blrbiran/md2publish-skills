...
3. **动手前先跑第三节那九条基线**（审计 0 条 / 审计变异 16 绿 / md2html 测试 25 绿 / 产物自检 PASS / 普查变异 71 绿 / 普查跑通 / theme_lib 单测 17 绿 / contrast_lib 单测 0 条失败 / 对比度变异 16 绿 + `prune_survivors`/`baseline_diff` 单测 0 条失败 + 对真实库跑一遍报 116 条不达标、116 条基线、无新增），确认没被上一轮改坏。
