...
> 更新于 2026-08-10（第七轮：对比度审计立项 + 两项清理）。仓库位置：`~/code/skills/writing/md2publish-skills/`
>
...
4. **普查这条线已收口**：`census-themes.py` 首轮报 43 条，分六个批次处理完，现在是 **0 条未销 + 8 条已豁免、exit 0**。逐批做了什么、每条豁免的理由、哪些是真修哪些只是销声，全在第六节第 1 条——**接着往下做之前先读那一节**，尤其是 1.7（两条被内容类型限定的规范，机械字段接不住）和 1.8（monochrome-mag 已整体删除）。新的活看第六节第 2–4 条。
...
9. **本轮的活在第六节第 6 条**（对比度审计）：spec 与实施计划都已就绪、**一行代码都还没写**。
   实测结论先记住一个数——**26 个主题里 18 个有对比度不达标组合，合计 113 种**，而普查报 0 条时
   对此一声不吭。这是第 5 条那句「0 不等于主题库成立」到目前为止最硬的一份证据。
...
> ⚠️ **排序例外：本轮新增的第 6 条价值最高，先看它。**它没有前移到第 2 位，是因为
> `docs/superpowers/specs/` 与 `plans/` 里有十处交叉引用指着现在的第 2–5 条编号，
> 前移会把那些指针全部打断——那些是存档，不该为一次重排去改（第零节第 4 条）。
...
- **一条待核的线索（2026-08-10 已核，结论比线索严重得多 → 见第六节第 6 条）**：
  下面这四处是当时顺带撞见的，那句「**没有做过全库对比度审计**」**现在已经不成立**——
  第七轮用探针实测出 **26 个主题里 18 个、合计 113 种**不达标组合，这四处只是其中几条。
  逐条数据不要照抄下面这段（它只覆盖人眼扫到的那几种组合），去看
  `docs/superpowers/specs/2026-08-10-contrast-audit-design.md`。原文留档如下：
...
### 6. 对比度审计：设计与计划已就绪，一行代码还没写（本轮新增，价值最高）

**这是第五节那句「0 不等于主题库成立」到目前为止最硬的一份证据。**本轮用一次性探针实测：
**26 个主题里 18 个有对比度不达标组合，合计 113 种**——而 `census-themes.py` 报 0 条时
对此从头到尾一声不吭。第六节 1.5 末尾那条「四处疑似违规、**没有做过全库对比度审计**」
的线索，实测下来低估了一个量级。

严重度分布：< 2.0 有 7 种（celadon-scroll 的 h2 饰线 1.45）、2.0–3.0 有 39 种
（morandi-fog 引用块正文 2.71、candy-pop 粉底上整套 2.89）、3.0–4.0 有 45 种
（autumn-warm 的 strong 3.54）、≥ 4.0 但未达标 22 种。

**两份文档已入仓，是这件事的权威来源，本文不复制细节：**

- 判据设计与理由：`docs/superpowers/specs/2026-08-10-contrast-audit-design.md`
- 实施计划（8 个任务、42 步、每步带完整代码）：`docs/superpowers/plans/2026-08-10-contrast-audit.md`
- SDD 工作区与 ledger：`.superpowers/sdd/2026-08-10-contrast-audit/progress.md`
  （gitignore，装着已做过的冲突普查、用户已裁决的三件事、执行顺序硬约束）

**用户已裁决、不要重新发明的四件事**：形态是常驻护栏不是一次性报告；装饰符号按图形 3:1 判；
**冻结基线、只挡新增**（存量不阻塞，`exit 1` 只在出现基线里没有的新组合时）；
走独立的第四套脚本，不并进 `census-themes.py`。

**三条最容易在实施时走偏的**：

1. **真实基数还没定。**113 是探针口径的**下限**，而探针的装饰判据（「没有字母/数字/CJK
   就算装饰」）正是 spec 否掉的那条——实测有 43 种组合会因此重新分类。真值预期在
   **113–156** 之间，要等计划 Task 6 首跑才能定。**在那之前任何地方都不许断言总条数**；
   首跑结果若落在区间外，停手上报，**不许调判据去凑区间**。
2. **装饰按注入来源判，不按字符类判。**字符类会把代码块里的 `{`、`|---|` 误放宽到 3:1，
   而代码里的符号是要读的。这与 `REF_EXCLUDE` 那次是同一个形状的错误，只是词表换成了字符类。
3. **别按文件名循环。**本轮探针用 `glob("*.html")`，把不在 `PAIRS` 里的
   `13-cyber-neon-v7-grid` 算成第 27 个主题，第一版数字（114 / 19 个主题）整体偏高。
   第四节早就写过这条纪律，照样踩了——**写过的纪律不等于会被执行，得让脚本自己去拿权威表**。

**处置那 113 条是另一轮的事，性质与普查那 43 条完全不同**：那 43 条大半靠改标签、改描述、
写注记就能销，产物逐字节不变；这一批几乎每条都要动配色、都会改产物、都含审美判断，
而且 candy-pop / washi-spring / morandi-fog 是整套配色系统性偏浅，不是一两个色的事。
**「冻基线」这个决定就是为此做的**——不要一上来就想着修到 0。
