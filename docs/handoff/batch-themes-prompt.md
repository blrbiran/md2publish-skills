...
## 三条判据（2026-08-05 实测补的，读了能省一轮返工）

### 1. 行内 code 用强调色文字 + 带底色 = 正常，不是冲突

本库 27 个主题里 **17 个**给行内 code 派了强调色文字，其中 15 个带淡底。2026-08 实测裁定过：
带底它就有自己的形状，和 strong 分得开，观感达标。审计脚本的 OVER 档就是按这个收窄的。

所以：**主题文件给行内 code 指定了强调色文字色，就照字面色值写，不要改成中性色，
也不要报成「与 `_common-tech.md` 冲突」。** 只有「**无底** + 强调色文字」才是真缺陷。

> 这条是第一轮试点用真金白银换来的：一个执行者以「`_common-tech.md` 优先」为由，
> 把 newsprint 的行内 code 从主题规定的报头红擅自改成了灰褐，整个主题要重跑。

### 2. 文末装饰有专门字段

主题文件若规定了「文末居中一枚印章 / 一行落款 / 「終」字」，用这两个字段：
...
**`footer` 的样式串必须自带 `color`**——铁律要求每个 `<p>` 有显式 color，脚本只自动补
`text-align`，不补 color。漏了会直接自检 FAIL（实测用了这个字段的主题 3/3 全踩）。
引用块首行前缀（如 `❝`）用 `blockquote_prefix_html`。

### 3. 这篇测试文没有导语段

它是 `# H1` 之后直接 `## 导读`，命中 `_common-tech.md` 的第三种情形：把首个 h2 的上间距
归零（`h2_first` 字段；不少主题的 h2 本来 margin-top 就是 0，那就不必加这个字段）。
主题文件里若写了「全文第一段做导语处理」，本文章用不上，报告里记一笔即可，**不要为此改文章**。
