...
> 更新于 2026-08-13 下半场（族 1 十三支注释灰／引用灰做掉）。**接手第一件事看第四节
> 「2026-08-13 下半场」**：87 条存量现场重量后聚成**四个族**（决策单元是族不是行），
> 其中族 1 那 13 支只挂 `blockquote`／`highlight.comment`／`em` 的低调色已按
> 「保色相保饱和、只压明度到 4.6」执行完毕，对比度基线 **87 → 67**，九条基线全绿。
> **剩下的活是族 2 那 9 支主强调色，每支都要用户单独拍板**；族 3（9 支被判据撞车污染的
> 装饰色）与族 4（3 支真机待定）已经用户裁定押后。
>
> 同一天上半场：B 档 13 支色里 7 支干净的已做掉（基线 110 → 87），并修了产物自检脚本的
> 一处 strip bug（它让一条新检查退化成只比字号、误判 6 个主题），
...
3. **动手前先跑第三节那九条基线**（审计 0 条 / 审计变异 16 绿 / md2html 测试 25 绿 / 产物自检 PASS / 普查变异 71 绿 / 普查跑通 / theme_lib 单测 17 绿 / contrast_lib 单测 0 条失败 / 对比度变异 16 绿 + `prune_survivors`/`baseline_diff` 单测 0 条失败 + 对真实库跑一遍报 67 条不达标、67 条基线、无新增），确认没被上一轮改坏。
...
9. **对比度护栏本轮已落地**（第六节第 2 条）：第四套机械脚本 `contrast-themes.py` + 冻结基线
   ...
   这道门没被锁死。**那个锚点数当时是 110，2026-08-13 上半场做完 B 档 7 支色后是 87、
   下半场做完族 1 十三支后是 67**（门若失效，aurora-flow 那 7 行会回来、数字变成 74）。
   （立项那一轮探针估的 113 是另一回事，用的装饰判据已被 spec 否掉，别再当事实抄。）
   ⚠️ **处置这 67 条的行动单元现在是「族」，不是原来的 B/C/D 档**——按挂载点聚，
   全库 35 支色分成四族，清单、状态与族 2 的建议顺序都在第四节「2026-08-13 下半场」。
   **下一步就是族 2 那 9 支主强调色，每支单独拍板。**
10. ⚠️ **另一个会话确实在同一分支上并行提交，这已不是「可能」。**图片 skill 那条工作线
    ...
    **共用 `main`**（原 `design/md2publish-image-skills` 已合入、本地只剩 remote）。
    ...
    （第四节那条提交事故就是这么发生的）。
    **2026-08-13 下半场再次实测有效**：开工时工作区干净，干到一半 `git status` 里冒出两份
    不是我改的文件（`docs/handoff/handoff.md` 与图片 skill 的 spec），`git commit -- <27 条
    具体路径>` 的提交仍然干干净净只有 27 个文件；那条线随后把自己那两份提交掉了。
    **所以别把「工作区干净」当成开工时确认过就一直成立的前提——提交前紧挨着再看一次。**
...
#    目前预期是 67 条不达标、67 条基线，无新增、exit 0
#    ⚠️ 数字沿革：116（首次落地）→ 110（2026-08-11 修掉 6 条判据假阳 + 1 条错键）
#       → 87（2026-08-13 上半场做掉 B 档 7 支色，剪掉 23 行）
#       → 67（同日下半场做掉族 1 十三支注释灰/引用灰，剪掉 20 行）。见第四节那三节
...
报 **67** 条不达标、全部已冻结进 `references/contrast-baseline.tsv`，exit 0。
...
（**2026-08-13 上半场做掉 B 档 7 支色后是 87、下半场做掉族 1 后是 67**，当前值一律以第三节为准），
...
2. **点名属性是错的修法。** 第一版修复只挡住上面那两个，复审随即找出同形的第三个：
   ...
   **验证这条白名单没有收得过窄，靠的就是基线那个数**：门若失效，aurora-flow 那 7 行
   会回来。当时是 110（失效则变 116）；2026-08-13 上半场做完 B 档 7 支色后是 87、
   下半场做完族 1 十三支后是 **67**（失效则变 74）。改这道门之后**必须重跑这一条**，不要只看单测绿。
...
13 支色里 7 支已于 2026-08-13 上半场做掉、基线降到 87，那一节还纠正了这张表的 4 个档位；
同日下半场又把剩下的 87 条按「族」重量了一遍、做掉族 1 十三支、基线降到 67，
...
token、四个主题的 em）；按文字口径重判，基线会在当时的 87 上涨到 99（族 1 做完之后的等价数字是 67 → 79——
这 12 行没有一行落在族 1 那 13 支色上，所以增量不变）。**方向是藏发现**，判据教训与
修法方向已写进 `docs/theme-design-lessons.md`「机械审计方法」节。

它对分档的污染，**下一个接手族 3 的人必须先看**（下半场重量后，这 4 支连同新查出的
5 支同形色一起归入族 3、已裁定押后，见下一节）：
...
（**下半场已按「族」重新组织并做掉族 1**，见下一节——C/D 这两个档名此后不再是行动单元。）
...
### 2026-08-13 下半场：87 条重量成四个族，做掉族 1 十三支，基线 87 → 67

**先做的是重量，不是改色。**上半场留下的「C 档 14 支 / D 档 15 支」是按差距大小分的，
但差距大小不决定**改起来是不是同一件事**。按上一节那个配方现场重跑一遍（87 行 / 35 支色，
C 14、D 15 与上半场对得上），再按**挂载点**聚一次，35 支色其实是四个族：

| 族 | 支数 / 行数 | 是什么 | 状态 |
|---|---|---|---|
| **1 注释灰 / 引用灰** | 13 支 / 20 行 / 356 处 | 只挂 `blockquote`、`highlight.comment`、`em`，**没有一支被当背景用** | **✅ 本轮做完** |
| **2 主强调色** | 9 支 / 43 行 / 1870 处 | 扛 h2 / strong / 行内 code / th，动一支改整篇观感 | **逐支拍板，未开工** |
| | | *（四族支数合计 34，比 35 支少 1：terracotta-sun 的 `#fdf8f1` 那 1 行不单独算一支，它是 `#c2593b` 同一个决定的另一头，见下方连锁）* | |
| **3 撞车污染的装饰色** | 9 支 / 21 行 | 名下「装饰」行里混着正文，按 3.0 压等于用假理由销真发现 | 用户裁定押后 |
| **4 真机待定** | 3 支 / 3 行 | retro-phosphor `#5a7a58`、aurora-flow h2 白字、cyber-neon-v7-edge `#ff4ba3` | 押后，挂第六节第 3 条 |

族 3 比上半场点名的 4 支多出 5 支同形的：candy-pop `#7fb5d5` / `#f28ba8`、lavender-dusk
`#b9a7d8`、celadon-scroll `#d8cfb8`、aurora-flow `#38c6d9`。**接手族 3 之前先把这 9 支
和上半场那张「4 支的 B 是假的」表合起来看**，它们是同一个口子。

**做了什么。**族 1 的 13 支按「HSL 保色相保饱和、只压明度到 4.6」执行
（midnight-study 是暗色主题，方向相反、**提亮**到 4.61）：

| 主题 | 旧 → 新 | 挂在哪 | 销掉 | 压完（最紧的那个底） |
|---|---|---|---|---|
| 03-spring-fresh | `#8fa08f` → `#617261` | 注释 | 1 行 18 处 | 2.48 → 4.60 |
| 08-morandi-fog | `#8c8884` → `#66635f` | 引用块 / 注释 | 2 行 41 处 | 2.71 → 4.61 |
| 02-ocean-calm | `#8a95a5` → `#626e7f` | 注释 | 1 行 18 处 | 2.72 → 4.64 |
| 25-washi-spring | `#8f8781` → `#6e6762` | 引用块 / 注释 | 2 行 41 处 | 2.96 → 4.66 |
| 10-lavender-dusk | `#8d8497` → `#6e6578` | 引用块 / 注释 | 2 行 41 处 | 2.99 → 4.64 |
| 04-ink-wash | `#8c887e` → `#6d6a61` | 注释（本来就带斜体） | 1 行 18 处 | 3.02 → 4.62 |
| 22-blueprint-grid | `#7189a0` → `#576d83` | 注释 | 1 行 18 处 | 3.12 → 4.61 |
| 17-scarlet-tech | `#8a8a8e` → `#6e6e72` | em / 注释 | 2 行 19 处 | 3.13 → 4.62 |
| 01-autumn-warm | `#a08876` → `#846d5b` | 注释 | 1 行 18 处 | 3.17 → 4.60 |
| 19-candy-pop | `#a3766f` → `#8b5f59` | 引用块 / 注释 | 2 行 41 处 | 3.33 → 4.62 |
| 18-midnight-study | `#96897a` → `#ada397` | em / 引用块 / 注释 | 1 行 1 处 | 3.35 → 4.61（**提亮**） |
| 24-botanic-press | `#77816d` → `#636b5b` | 引用块 / 注释 | 2 行 41 处 | 3.41 → 4.63 |
| 14-celadon-scroll | `#7c7365` → `#6f665a` | 引用块 / 注释 | 2 行 41 处 | 3.82 → 4.61 |

13 支的 `.md` 都按**规则 11** 补了注记，写明它在每个底上的比值、哪一处是下限、压之前差多少。

⚠️ **改 `.md` 时要把该 hex 的每一处都换掉，包括只活在散文里的那几处**——botanic-press 的
h2 小叶饰 `❦`、celadon-scroll 的诗词引文、scarlet-tech 的 `＋/－` 符号对都引用这支色，
但 `theme.json` 里并没有对应字段（它们是第六节第 6 条「明确不做的」那一类）。只改调色板行
会让组件规范里留下一个调色板未声明的色，`audit-themes.py` 立刻报 `DESYNC`。

**验证（九条基线全绿）**：产物里旧色 **13 份全 0 处**、新色 18–24 处；全仓库旧 hex 0 残留；
基线 diff **只减 20 行、增 0 行**，20 行全部落在这 13 支旧色上；`audit-themes.py` 0 条、
`census-themes.py` 0 未销 + 8 豁免、`contrast-themes.py` 67 条 / 67 基线 / 无新增 / exit 0、
26 份产物自检全 PASS。⚠️ 与上半场同理，`test-md2html.sh` 的 PART B 这一轮对这 13 份**是
同义反复**（先按纪律重生成产物再跑），真正的证据是上面那三组数。

**顺带现场量出来的一条连锁，交给族 2 的人**：terracotta-sun 的 `#c2593b` **既是 strong 的
前景、又是 h2 的底**，压暗它 strong 从 3.39 升到 4.61 的同时，h2 白字 `#fdf8f1` 从 4.15 一并
升到 **5.65**——上半场记的那条「`#fdf8f1` 换纯白也只有 4.39、不够」的死结，**从底那一侧解开
就没了**，一支色销 4 行。aurora-flow 的 `#6a5cff` 同形（白字 4.58 → 5.46）。所以
「被当背景用 ⇒ 压明度修不了」这条**只对「它是别人的底、自己不做前景」的色成立**，
`#fdf8f1` 那种才是；既做前景又做底的，压暗对两头都有利，别被这条挡住。

**族 2 的建议顺序**（未拍板，每支仍要单独定）：`23-terracotta-sun #c2593b`（连锁，销 4 行）
→ `08-morandi-fog #b08e8a`（2.29:1，全库最不可读的正文强调）→ `19-candy-pop #d96687`
（2.89，273 处）→ `01-autumn-warm #c06b4d`（308 处）→ `25-washi-spring #b56b7d` →
最后 4 支各只 2–3 行的（`16-arena-charge #e03131`、`21-aurora-flow #6a5cff`、
`10-lavender-dusk #7f6a9e`、`22-blueprint-grid #b26a1b`）。**candy-pop / washi-spring /
morandi-fog 是「整套系统性偏浅」**，压单支色可能只是把问题挪到另一支，候选值要看着定。

**这一轮学到的通则已回写 lessons**（「压暗必然压缩明度分层，这是浅底 + AA 的数学必然」
那条判例），不在此复制。
...
- **一条线索，现已被机械覆盖（2026-08-10 已核，结论见第六节第 2 条）**：
  ...
  `references/contrast-baseline.tsv` 现收了 **67** 条不达标组合（首次落地时是 116，
  2026-08-11 修正判据后降到 110，2026-08-13 两轮处置降到 67，见第四节），这四处只是其中几条。
...
7 支色、下半场又做掉族 1 十三支 20 条，余 67 条**，见第四节那两节）：43 条大半靠改标签、改描述、写
...
2. **`contrast-ok` 豁免注记的解析故意没做**——存量里（当时 110 条，现 67 条）没有一条已判定「可以永远这样」，
...
- `superpowers:brainstorming` + `AskUserQuestion` — 处置族 2 时逐支色跟用户对齐再动手；B 档与族 1 两轮都是先出「候选值 + 挂载点 + 影响面 + 观感代价」的表、拿到裁决才改文件，中途因此拦下过 4 支档位不实的色。**别把 9 支主强调打包成一个问题**：族 1 那 13 支形态完全同构、可以整族一次拍板，族 2 每支改的都是整篇观感，不同构
- `tech-writer` — 校订教程文档时
