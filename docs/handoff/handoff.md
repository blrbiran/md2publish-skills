...
> 更新于 2026-08-11 下半场（文档收尾）。**接手第一件事看第四节「2026-08-11 下半场」**：
> 对比度基线原有 116 条，查出 6 条是判据假阳（`contrast_lib.py` 不认 `background-size`）、
> 另 1 条底记错了。**Task 1（新判据 `image_reaches_text`）与 Task 2（基线 116→109→110）
> 已执行并入仓**——`contrast-themes.py` 现对真实库跑出 **110 条不达标、110 条基线、
> 无新增、exit 0**。同一轮出的 110 条处置分档清单仍在（见第四节）。
>
...
3. **动手前先跑第三节那九条基线**（审计 0 条 / 审计变异 16 绿 / md2html 测试 25 绿 / 产物自检 PASS / 普查变异 71 绿 / 普查跑通 / theme_lib 单测 17 绿 / contrast_lib 单测 0 条失败 / 对比度变异 16 绿 + `prune_survivors`/`baseline_diff` 单测 0 条失败 + 对真实库跑一遍报 110 条不达标、110 条基线、无新增），确认没被上一轮改坏。
...
#    目前预期是 110 条不达标、110 条基线，无新增、exit 0
#    ⚠️ 116 是判据修复前的旧值，已于 2026-08-11 修正为 110（中途 `--prune` 会先出现
#       109，那是过程值不是算错）。见第四节「2026-08-11 下半场」
...
报 **110** 条不达标、全部已冻结进 `references/contrast-baseline.tsv`，exit 0。
...
- **一条线索，现已被机械覆盖（2026-08-10 已核，结论见第六节第 2 条）**：
  ...
  `references/contrast-baseline.tsv` 现收了 **110** 条不达标组合（首次落地时是 116，
...
2. **`contrast-ok` 豁免注记的解析故意没做**——110 条里没有一条已判定「可以永远这样」，
