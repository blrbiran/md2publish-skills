...
4. **下一件事看第六节第 1 条**：产物落点普查脚本（`census-themes.py`）首轮对真实库报出 **43 条**，因此 **exit 1**。批次 1a 处理 6 条（5 条豁免注记 + celadon-scroll 补 `h2_suffix_html`）、批次 1b 处理 20 条（收窄 `NEAR-ZERO` 判据 18 条 + gilded-ink 两支现造色转正）、批次 2 处理 7 条（3 条 UNMOUNTED 补字段 + terracotta-sun 注释色退回正文色 + autumn-warm/ocean-calm/spring-fresh 主副强调标签对调）、批次 3 处理 6 条（bauhaus-pop 错值 + terracotta-sun keyword 对比度 + gilded-ink/candy-pop 装饰色改标签 + mint-breeze 阈值擦边豁免〔批次 5 已撤回〕 + mint-breeze/botanic-press 两条判断层规范改写并豁免）、批次 4 处理 2 条（`INVERT` 改判据只比散文面，销掉 terracotta-sun 那条 + candy-pop `NEAR-ZERO` 写豁免注记），外加整体删除 monochrome-mag 主题带走的 2 条，批次 5 撤回批次 3 给 mint-breeze 写的那条 `INVERT` 豁免注记（理由不实，见下）后重新报出，与批次 4 新暴露的 `10-lavender-dusk` 同为一形；批次 6 把这两条一并处理——查挂载表确认两者都是**标签错了不是渲染错了**：被标成「主强调」的那支只挂在 h3 前缀、列表前缀、em 与代码高亮上（lavender-dusk 散文面 9/219、mint-breeze 16/93），真正扛 h2 与 strong 的是 `#5d4b7c` 与 `#1e7a5c`（散文面 73 / 78）。按批次 3 给 gilded-ink、candy-pop 定下的写法改标签，`theme.json` 一字未动、产物逐字节不变。现在是 **0 条未销 + 8 条已豁免、exit 0**。**这个 0 只覆盖「声明的色在产物里有没有落点」这一层，不代表主题库成立**——见第三节基线 6 下面那段。第六节第 1 条装着分组、执行顺序和最要紧的几条的全部细节，逐组过一遍、拍板一批、改一批、再跑一遍 `census-themes.py` 确认条数下降。
