...
>
> 1. **这是一份建议清单，不是裁决记录。** 除下面标了「✅ 已处理」的各条外（截至批次 4，43 条里只剩
>    §7.x 的 cyber-neon `alert` 一条未办；批次 4 另新暴露一条不在本文件里的 `INVERT 10-lavender-dusk`），其余条目
...
>
> **✅ 批次 3 进度更新（2026-08-09）**：又有 **6 条**被用户拍板并执行——§四 bauhaus-pop 的
> 错值（**改 `.md`**，产物不变）、§6 的 `INVERT 09-gilded-ink` 与 §9.3 的
> `INVERT 19-candy-pop`（**改角色标签**，但**不是对调**——压过它们的色本来就标着
> 「strong 用」，错的是把装饰色叫「主强调」；产物不变）、§6 的 `INVERT 15-mint-breeze`
> （**写豁免注记**，93 vs 288，三分之一线 96，只差 3 处）、§3.5 与 §9.2 的两条
> `list_prefix_ol_html`（**改写规范为「判断层手工可选项」+ 写豁免注记**，产物不变）。
> 另外处理了一条本文件记为「未处置」的线索：§5.2 末尾的
> `terracotta-sun highlight.keyword #c2593b`（代码底 `#efe0cd` 上 3.39:1，**已复算**）
> 走规则 9 第二步退回 `#4f382b` + `font-weight: 700`，产物已重生成。
> **同一批还整体删除了 monochrome-mag 主题**（用户裁定永不使用），§9.1 那两条随之消失，
> 主题库 27 → 26，见 `handoff.md` 第六节 1.8。
> **普查现在报 2 未销 + 8 已豁免（仍 exit 1）。**
>
> ⚠️ **本文件 §四 有一处事实错误，就地更正**：那里写「`#005baa` 在 `#171614` 上是
> **2.51:1**（我算的）」——复算是 **2.6495 → 2.65**，与原句里的数字一致，所以批次 3
> 只改了色值、没有改数字。（`#be1e2d` 的 2.94 复算无误，正是它验证了这次的算法。）
>
...
>
> **✅ 批次 3 已处理（走 (b) 并加强）**：规范行重写成**显式声明为判断层的手工可选项、
> `theme.json` 刻意不配该字段、机械路径退回纯文本 `N.`**，再写一条
> `census-ok: UNMOUNTED list_prefix_ol_html` 注记。**只写豁免不够**——那句话读起来仍像
> 机械规范，下一个执行者还是会去配字段；把「这是判断层的活」写进规范本身才真正修正了描述。
> 上面那个 `width: 20px` 的两位数撑破问题也写进了规范行（手工用到两位数改 `min-width`），
> 下面那段 `theme.json` 片段**不要照抄**。与 §9.2 的 botanic-press 同一处置。
...
> **✅ 已处理（真改 `.md`，批次 3）**：色值已改成 `#005baa`，`theme.json` 未动、产物逐字节不变。
> **下面那句「`#005baa` 在 `#171614` 上是 2.51:1」是错的**——复算为 **2.6495 → 2.65**，
> 与规范行里原有的数字一致，所以只改了色值、数字保持 2.65。同一次复算里
> `#be1e2d` = 2.9390 → 2.94，与原句吻合，正是这一条验证了算法没写错。
> 顺带把底色写进了那句话（「在墨黑底 `#171614` 上」），免得下一个人还要去翻是哪个底。
...
> ⚠️ **同一次量到、当时未处置**：`highlight.keyword` `#c2593b` 在同一个代码底上只有
> **3.39:1**，同样低于 4.5——它与 §6 的 `INVERT #c2593b` 是同一支色。
>
> **✅ 批次 3 已处理这一条**：复算确认 3.3878。该代码底 `#efe0cd` 上整个调色板只有
> `#4f382b`（8.3880）与 `#8f3f28`（5.5726）过 4.5，而 `#8f3f28` 已被 `string`/`key` 占住、
> 橄榄绿 `#6f7a4d`（3.5503）既不达标又被 `terracotta-sun.md:45` 的分寸条款限死——
> **没有第二个选择**，只能按规则 9 第二步退回正文色 `#4f382b` 并靠 `font-weight: 700`
> 与注释（italic）、数字（常规）区分。`terracotta-sun.md` 同步补了一行语法高亮条款，
> 把这条取色链路和「不要为了让代码块有颜色把它们调回陶橙」写死。
> **这一条没有解决 §6 的 `INVERT #c2593b`，反而加重了它**：赤陶橙的文字落点从 86 降到 69，
> 参照色深陶红仍是 284，那条 INVERT 照报。这正是 §6 末尾预告过的
> 「先定对比度、再回头看 INVERT」，两件事确实是分开的。
...
>
> **✅ 批次 3 处理了其中两条，且两条走了不同的路：**
>
> - **09-gilded-ink `#b08a3e`：改角色标签，不是豁免。**下面为它写的注记文本作废。
>   与 01/02/03 的对调也不同——**压过古金的 `#8f6f2f` 本来就标着「深金（strong 用，更沉）」，
>   不是「副强调」，没有可对调的对象**；错的是把一支纯装饰色叫作「主强调」。实测总 43 处
>   = 文字 16（◆ 前缀）+ 线 27（h2 上下金线、引用块左线），改标「古金（金线与饰记，装饰用）」
>   并写明落点分工。`theme.json` 未动，产物逐字节不变，INVERT 自动消失。
> - **15-mint-breeze `#2fa47e`：按下面的建议写了豁免注记，但理由重写过。**下面那条注记
>   说「薄荷绿 3.12:1 不做正文色」，这个理由在这里是**多余且误导的**——它有 93 处货真价实的
>   文字落点，触发 INVERT 的原因不是它弱，是**阈值擦边**：深叶绿 288，288÷3 = 96，只差 3 处。
>   落地的注记写的是这三个数字。
>
> **六条 INVERT 最后只有一条真的走了豁免。**遇到 `INVERT` 先问「是不是标签错了」——
> 这一批又坐实了一次 lessons 那条判例。
>
> **✅ 批次 4 处理掉最后一条（23-terracotta-sun `#c2593b`），走的是第四条路：改判据。**
> 下面为它写的注记文本作废——**不写豁免，因为倒置的既不是标签也不是设计，是判据本身**。
> 那句注记文本其实已经摸到了正确答案的边（「284 处里约 190 处是行内 code 文字」），
> 却把它当成了写豁免的理由，而不是「判据在量错东西」的证据。
>
> 逐色拆代码面 / 散文面后（2026-08-09 实测）：
>
...
>
> **行内 code 各贡献恰好 193 处——它是语料的常数，不是主题的属性。**`INVERT` 因此改成
> 只比散文面（`<pre>` 块 + 行内 code 不计），terracotta-sun 那条自动消失（主强调散文面 69
> 对参照色 14）。`theme.json` 与主题 `.md` 均未动，产物逐字节不变。
> **同一次改动新暴露出 `INVERT 10-lavender-dusk #7f6a9e`**（散文面 9 对 73），
> 旧判据被那 193 处喂饱、从没报过它——见 `handoff.md` 第六节进度块 4c。
>
> ⚠️ **顺带作废了 15-mint-breeze 那条已落地注记里的三个数字**：换成散文面口径后是
> 16 对 78、三分之一线 48，不再是「只差 3 处」；而且原注记把 93 处全归给「h3 前缀 ✓、
> 无序列表前缀 ✓」也是错的，其中 77 处是代码块内的 `highlight.string`/`key`。
> 注记仍然销得掉那条发现（不报 `STALE-NOTE`），**所以脚本不会提醒任何人**——
> 批次 4 按「不重访已豁免发现」的边界没有动它，**需要用户重新裁决一次**。
...
> **✅ 已处理（批次 3）：用户裁定这个主题永不使用，整体删除。**下面 (A)/(B) 两条路
> **都作废**——两条发现随主题一起消失，**这是「问题不存在了」，既不是修复也不是豁免**。
> 主题文件与 `theme.json` 已移进废纸篓，全部引用点（INDEX、`PAIRS` 表、两处脚本注释、
> 各处「27 个主题」计数）已逐个清掉并核过无悬空。lessons 规则 5/15 里的案例与类比**保留**
> ——那是历史证据，删掉会砍掉规则本身的依据。逐项清单见 `handoff.md` 第六节 1.8。
...
> **✅ 已处理（批次 3）：既不兑现也不只写豁免，而是「改写规范 + 豁免」。**下面「略微倾向
> 兑现」的建议**未被采纳**——`list_prefix_ol_html` 是全局字段，配上去每篇文章的每个有序列表
> 都会套上 `No.`，而「这个列表是不是物种/条目清单」是内容语义，机械层判不了（与 §3.5
> 的 mint-breeze 同形，见 `handoff.md` 1.7）。下面「若拍豁免，建议把『可用』改成明确的
> 否定或删掉整个从句」这半条被采纳并加强：规范行重写成**显式声明为判断层的手工可选项、
> 机械路径退回纯文本 `N.`**，判断层的指引因此加强而不是丢失。产物逐字节不变。
...
> **✅ 已处理（批次 3，走 (a) 改标签）**，但**下面 (a) 的写法只对了一半**：它说「把樱粉与
> 深樱粉的角色写法**对调**」——**对调不成立**，深樱粉本来就标着「strong 用」，不是「副强调」，
> 没有可换的对象。落地的改动只有一处：`candy-pop.md:15` 的「樱粉（主强调）」改成
> 「樱粉（符号与前缀装饰用）」并写明 29 处文字落点全在 h3 圆点 / 列表前缀 / 代码关键字上。
> 深樱粉那一行一个字没动。`theme.json` 与产物逐字节不变，INVERT 自动消失。
> (c) 那条「三个色都不过对比度门槛」仍然成立，仍待真机定夺——**改标签没有碰配色**。
...
> **✅ 已处理（批次 4，写豁免注记）——本节的判断被推翻了，留档是为了说明推错在哪一步。**
> 翻转它的是一行样式串：`candy-pop.md:36` 的 `em` 是
> `background-color: #e8f2f9; padding: 1px 5px; border-radius: 4px; color: #4a7a9b`，
> **没有 `font-style: italic`**——这个主题的 `*文字*` 根本不渲染成斜体，是一枚圆角浅蓝
> 高亮块。用户确认高亮是常用写法，于是「不用斜体 ⇒ 永远渲染不出来 ⇒ 规则 1 死色」
> 这条链在第一环就断了。产物 1 处是因为**本篇语料通篇只有 1 个 `*em*`**
> （实测 `<em` 1 次、`#e8f2f9` 1 次），是语料的性质，不是落点失效。
> 下面三条候选路**一并作废**，`candy-pop.md` 的样式与 `theme.json` 都没动。
