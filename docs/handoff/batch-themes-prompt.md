...
> 更新于 2026-08-06：**`theme.json` 已经全部入仓**（26 份），这份文档的适用范围因此大幅收窄——
> 先看第一节判断你到底需不需要开新会话。
...
| 场景 | 怎么做 | 成本 |
|---|---|---|
| **换一篇文章，想看 26 个主题的效果** | **不需要开新会话**，直接循环跑脚本（第二节） | **$0** |
| **改了少数几个主题文件，要同步 theme.json** | 自己在当前会话改那几份，别开批量 | 很低 |
| **改了 `_common-tech.md` 或铁律，26 份 theme.json 都要重做判断** | 开新会话，用第三节的 prompt | $25–35 |
...
26 份全部应当 PASS。**有 FAIL 说明这篇文章命中了某个主题没覆盖到的结构**（比如出现了有序列表、提示卡、导语段，而该主题没配对应字段）——那才需要人去看，而且只需要看 FAIL 的那几个。
...
本库 26 个主题里 **17 个**给行内 code 派了强调色文字，其中 15 个带淡底。实测裁定过：带底它就有自己的形状，和 strong 分得开，观感达标。审计脚本的 OVER 档就是按这个收窄的。
...
- 产物目录里已有全部主题的旧产物（外加已删除的 monochrome-mag 那份孤儿产物），**不要删任何文件**
