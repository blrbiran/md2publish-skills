...
<!-- audit-ok: OVER #3d6a8a 2026-08 实测：石蓝与正文深蓝灰同色系、明度接近，上百处行内 code 不构成斑点感，用户确认观感可接受 -->

蓝色的高频落点已由 h2 符号、h3 短线、列表前缀、引用边框保底。行内 code 的文字色也是石蓝，这是本主题的例外：它占了石蓝落点的大半，一般情况下属于落点过载（规则 7），此处成立是因为石蓝与正文色同属深蓝灰、明度接近，密集出现也不跳。**别把这个例外套到对比强烈的主题上**——同样的写法在 cyber-neon 上就让行内 code 和 strong 变得无法区分。本主题适合"讲道理"的技术分析文；如果文章代码块密集、需要编辑器级语法高亮，改用 `editor-slate`。
