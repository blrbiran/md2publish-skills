...
| `md2publish-visuals` | 正文配图 / 信息图 / 卡片系列，真调 provider 生成，**回写 Markdown**（另存 `article.illustrated.md`） | 本地文件 + **API 消费（花钱）** | provider API key（缺失时降级为只产 prompt） |
...
- **「配图零副作用」已作废**：那是旧计划模式的口径。`md2publish-cover` 真调 provider、
  真花钱、不可逆；`visuals` 还会回写 Markdown（另存 `article.illustrated.md`，
