...
- 制图蓝（主强调）：`#1d5a8a`
- 批注橙褐（副强调）：`#b26a1b`
- 次级灰蓝：`#576d83`（图注、尺寸标注、次要说明段的文字色——声明即须使用，别让它成死色）
> 次级灰蓝的明度是被对比度铁律钉住的：在代码底 `#e7eff7` 上 4.61:1、白卡上 5.36:1、引用底 `#fdf8f0` 上 5.07:1。**代码底那一处就是它的下限**——2026-08 为此压深过一档（压之前 3.12:1，差 1.38 不达 AA），不要调回去（规则 11）。
- 网格/边线：`#c9d9e8`
- 浅蓝底：`#e7eff7`
...
- 批注框标题 / 警示性 strong（原文里带"注意、警告、易错"语义的）/ 图注编号：`color: #b26a1b`
- 普通 strong 保持主强调蓝。批注橙褐的文字落点不能只有斜体——中文技术文全篇斜体常常是零，那样这个色就只剩虚线框，读者感觉不到它存在
- 次级灰蓝 `#576d83` 的落点：图注、尺寸标注、次要说明段、代码块内的注释。**必须真的用上**——声明了却在组件规范里找不到落点的颜色等于不存在
