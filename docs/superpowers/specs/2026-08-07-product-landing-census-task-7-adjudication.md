...
>
> 1. **这是一份建议清单，不是裁决记录。** 除下面标了「✅ 已处理」的各条外（截至批次 2 共 33 条），其余条目
>    **没有一条经过用户拍板**——原样保留当时的建议文本，读的时候当待办，不要当结论。
> 2. **本文件是临时文件，会被删除。** 待它记录的条目全部处理完（或被拍板为不处理）后，
>    应当删除本文件；不要把它当长期文档来维护或引用。
> 3. **`docs/handoff/handoff.md` 第六节第 1.1–1.7 条是权威摘要。** 那是本文件溶解、压缩
...
>
> **✅ 批次 2 进度更新（2026-08-09）**：又有 **7 条**被用户拍板并执行——§3.1/3.2/3.3 三条
> UNMOUNTED（**补 `theme.json` 字段**，产物已变）、§5.2 的 INVENTED（**退回正文色 + 斜体**，
> 产物已变）、§6 里 01/02/03 三条 INVERT（**改走「对调调色板角色标签」而不是写豁免注记**，
> 产物逐字节不变）。§3.5 的 `mint-breeze list_prefix_ol_html` **复核后退回用户**：规范限定
> 「步骤类」，机械字段接不住，见该节与 `handoff.md` 1.7。
> **普查现在报 10 未销 + 5 已豁免（仍 exit 1）。**
>
...
> **✅ 已处理（真修，批次 2）**：字段已补，HTML 已重生成，`test-md2html.sh` 已过。
> **但落地写法与下面的建议不同**：`ink-wash.md:47` 的主写法是
> `<p style="text-align: center; color: #b5432a; font-size: 18px; margin: 40px 0 0;">□</p>`，
> 带边框的「完」字只是同句里「可换为」的备选。执行时取了**主写法**，所以
> `footer_html` 就是 `"□"`，没有内层 `<span>`、没有 `inline-block`，下面那条陷阱天然绕开。
...
> **✅ 已处理（真修，批次 2）**：按下面的建议逐字执行。实测语料里 **4 处 strong 变品红**，
> 是「品红在正文里 0 处文字落点」这条立项缺陷的第一次真正兑现。
...
> **✅ 已处理（真修，批次 2）**：按下面的建议逐字执行。⚠️ 与 3.2 不同，本语料里
> **0 处命中**（没有带「注意/警告/易错」的 strong），产物逐字节不变——**字段就位不等于
> 已被验证渲染**，换一篇含警示语的文章才看得到。
...
> **⛔ 批次 2 复核后未执行，退回用户。**规范原句限定了**内容类型**（「**步骤类**有序列表」），
> 与 §9 里 botanic-press 那条「物种/条目清单**可用**褐色序号」同形；`list_prefix_ol_html`
> 是全局字段，配上去会给每篇文章的每个有序列表都套圆底徽章，机械层无法识别「步骤类」。
> 另一条独立的机械理由：这个徽章是 `width: 20px; border-radius: 50%` 的**单位数**圆，
> 序号到两位数就撑破。三条出路与取舍见 `handoff.md` 第六节 1.7。
> ⚠️ 语料里一个有序列表都没有，所以「产物不变」在这里**不能**当作验证通过。
...
> **✅ 已处理（真修，批次 2）**：按下面的建议逐字执行，`highlight.comment` 改成
> `color: #4f382b; font-style: italic;`。四个对比度数字（2.58 / 8.39 / 5.57 / 3.55）
> 与 `terracotta-sun.md:45` 的橄榄绿分寸条款均已独立复算/复核属实
> （**下文写的行号 `:46` 是笔误，实际在 `:45`**）。
> ⚠️ **同一次量到、未处置**：`highlight.keyword` `#c2593b` 在同一个代码底上只有
> **3.39:1**，同样低于 4.5——本批未获授权动它，它与 §6 的 `INVERT #c2593b` 是同一支色。
...
> **⚠️ 批次 2 对其中 3 条改走了另一条路，下面的注记原文对这 3 条已作废。**
> 01-autumn-warm / 02-ocean-calm / 03-spring-fresh 三条**没有写豁免注记**，而是
> **对调了各自 `.md` 调色板里的「主强调」/「副强调」标签**：三份 `theme.json` 已逐一核对，
> 标为「副强调」的深色实际挂在 h2/h3 文字、`strong`、行内 code、表头、代码高亮上
> （文字落点 317 / 333 / 317），标为「主强调」的亮色挂的是 h2 符号、h3 短线、列表前缀、
> 引用边框、em（各 21）——**倒置的是标签，不是设计**。对调后 INVERT 自动消失，
> `theme.json` 未动、**产物逐字节不变**（已核）。
>
> 这与「写豁免注记」的区别值得记：豁免是承认「主强调确实没什么文字落点，但有正当理由」；
> 对调标签是指出「这支色根本就不是主强调」。**后者修正了描述，前者只是让脚本闭嘴。**
> 剩下 09-gilded-ink / 15-mint-breeze / 23-terracotta-sun 三条仍按下面的建议待办
> （它们的两支色不是同一族的深浅关系，不能照搬对调）。
