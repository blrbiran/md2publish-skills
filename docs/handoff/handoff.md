...
> 更新于 2026-08-13（对比度 B 档做掉 7 支色）。**接手第一件事看第四节
> 「2026-08-13」**：B 档 13 支色里 7 支干净的已按「压明度到刚过阈值」执行完毕，
> 对比度基线 **110 → 87**，九条基线全绿。剩下的活是 C/D 两档，**每支色都要用户单独拍板**。
> 同一轮还修了产物自检脚本的一处 strip bug（它让一条新检查退化成只比字号、误判 6 个主题），
> 见第三节第 4 条——那一条现在还记着自检的另一个坑：**报 FAIL 时退出码是 0**。
>
> ⚠️ **同一轮量出一条判据缺陷，已记录、未修**：`is_decor()` 按样式串逐字相等认装饰，
> 会撞上正文——arena-charge 的 `strong` 与它 `h3_prefix_html` 注入的 span 样式一字不差，
> 61 处正文 strong 因此按图形 3:1 判。全库 **12 行 / 123 处**正文被这样降格。
> **它污染分档**：B 档那 13 支里有 4 支的「B」是假的（详见第四节 2026-08-13 那节与
> lessons）。用户已裁定这一轮只记录、改判据另开一轮。
>
> 上一轮（2026-08-12）：`background-size` 那条线全部收口——基线原有 116 条，查出 6 条是
> 判据假阳（`contrast_lib.py` 不认 `background-size`）、另 1 条底记错了，修正为 110。
>
> ⚠️ **判据现在是「闭世界」的，改它之前先读第四节那段**：`image_reaches_text()` 只在
> 元素声明的每一个 `background-*` / `padding-*` 属性都在它建模的那一小撮里时才往下判，
> 不认识就直接倒向「保留图像」。**别再往里加针对某个属性的特判**——那正是 review
> 两次抓到的错误修法。
>
> ⚠️ **同一天还发生一次语料事故，接手前先知道**：`~/code/skills/writing/wechat_test/`
> 整个目录连同 26 份定稿 HTML 一度完全消失，第三节九条基线里有四条（第 3、4、6、9 条）
> 曾因此 SKIP。已从归档副本
> 重建语料、用 `test-md2html.sh` 的权威 `PAIRS` 表重新生成全部 26 份 HTML——**但重建后的
> 文章比原文少了一处 `<strong>`**，第四节分档表里几个色的处数比今天的真实语料高 1，
> 已在原地标注，**不要去改那几个历史数字**。详见第四节「2026-08-11 下半场」。
>
...
3. **动手前先跑第三节那九条基线**（审计 0 条 / 审计变异 16 绿 / md2html 测试 25 绿 / 产物自检 PASS / 普查变异 71 绿 / 普查跑通 / theme_lib 单测 17 绿 / contrast_lib 单测 0 条失败 / 对比度变异 16 绿 + `prune_survivors`/`baseline_diff` 单测 0 条失败 + 对真实库跑一遍报 87 条不达标、87 条基线、无新增），确认没被上一轮改坏。
...
9. **对比度护栏本轮已落地**（第六节第 2 条）：第四套机械脚本 `contrast-themes.py` + 冻结基线
   ...
   ⚠️ **基线原有 116 条，其中 6 条是判据假阳、1 条底记错了，2026-08-11 已修正为 110**——
   `image_reaches_text()` 判据与基线下降都已执行并入仓，见第四节「2026-08-11 下半场」。
   那节末尾还记着**改这道门的两条硬纪律**：不许再加针对某个 CSS 属性的特判（它是闭世界
   白名单），以及改完必须重跑 `contrast-themes.py` 看数字对不对——单测全绿证明不了
   这道门没被锁死。**那个锚点数当时是 110，2026-08-13 做完 B 档 7 支色后是 87**（门若
   失效，aurora-flow 那 7 行会回来、数字变成 94）。
...
10. ⚠️ **另一个会话确实在同一分支上并行提交，这已不是「可能」。**图片 skill 那条工作线
    ...
    **共用 `main`**（原 `design/md2publish-image-skills` 已合入、本地只剩 remote）。
    本轮期间它往 `main` 插了十来个提交，**插在本计划的提交中间**——两条线在 `main` 上是交错的，
    没有哪个连续区间能单独表达其中一条。它历史上做过 `add -A` 式提交、卷走过别人未提交的改动。
    所以：**开工前先 `git log` 看分支有没有在你不知情时前进；提交只 `git add <具体路径>`，
    永远不要 `git add -A`；要评审自己这条线的改动，按路径过滤而不是按提交区间。**
    ⚠️ **2026-08-13 起「按路径分线」也不再成立**：那条线开始改 `md2publish-article` 自己的
    文件了（给 `references/wechat-html.md` 的自检加了一条检查，见第三节第 4 条）。本轮它在
    我干活的一个多小时里往 `main` 落了五个提交，工作区里还一直带着它**未提交**的改动
    （`scripts/test-sync-drift.sh` 等）。**提交一律用 `git commit -- <路径>` 逐条列路径**，
    本轮实测这样能把它的未提交改动挡在外面；`git add <路径>` + `git commit` 挡不住
    （第四节那条提交事故就是这么发生的）。
11. ⚠️ **`git log`/`git diff` 带 pathspec 时会被 `rtk` 代理吃掉。**本会话实测：
    `git log <范围> -- <路径>` 经代理返回空，**看起来像「这些路径没被碰过」而不是报错**——
    一个静默的错答案。要按路径查历史就走 `/usr/bin/git`。
...
| **`docs/theme-design-lessons.md`** | **改任何主题文件之前必读**；改审计/普查脚本前读「机械审计方法」节 | **只增不删** | 判据与规则（规则 1–16 + 案例 + 判例） |
...
#    目前预期是 87 条不达标、87 条基线，无新增、exit 0
#    ⚠️ 数字沿革：116（首次落地）→ 110（2026-08-11 修掉 6 条判据假阳 + 1 条错键）
#       → 87（2026-08-13 做掉 B 档 7 支色，剪掉 23 行）。见第四节那两节
...
第 3 条的 PART B 从 `references/theme-json/` 读 theme.json、与实验目录里的定稿 HTML 逐字节比对。**故意改了某份 theme.json 之后要先重新生成它的 HTML 再跑**，否则那里报的红是预期内的改动，不是回归——别反过来改测试迁就它。语料目录缺失时 PART B 会整体 SKIP 并把退出码标红（静默跳过等于没有护栏）。⚠️ **2026-08-11 这一轮的 26/26 绿不算回归证据**：语料目录当天整个消失又重建过。PART B 的机制是拿仓库的 `theme.json` 现场重跑一次 `md2html.py`，把这次输出与语料目录 `out/` 里的「定稿」HTML 逐字节比对——**两侧是同一份代码在同一组输入上的两次独立运行，不是一次运行产出的两份拷贝**。这一轮两次运行的输入都是重建之后的同一批文件，所以比对**仍能抓到两次运行之间的不确定性**，但**证不了「相对某次编辑，产物没有变化」**——因为没有一份独立于今天重建之外的旧基线可比，这条护栏要等下一次真正改动起才重新有牙齿。详见第四节「2026-08-11 下半场」的语料事故记录。

第 4 条有两个坑，2026-08-13 各踩到一次。**一是它报 FAIL 时退出码仍然是 0**——`print('FAIL:…')`
之后没有 `sys.exit(1)`，所以**必须读输出，不能只看退出码**（用 `| tail -1` 判是不是以 `PASS`
开头；这条工程债没修，见第六节第 2 条）。**二是它会被另一条工作线改**：那条线给自检加过一条
「导语段不该和 h3 同字号同字重」的检查，其 `_key()` 拆样式串时没有 strip 字典的键，
`font-weight` 永远取不到值、检查退化成只比 font-size，6 个主题（coffee-journal /
celadon-scroll / midnight-study / blueprint-grid / washi-spring / retro-phosphor，**都没配
`p_first`**）被误判 FAIL。本轮已修（单独一个提交），并双向验过：26 份产物全 PASS、同字号
同字重的探针照报、字重不同的不报。**下次看到自检突然报一批没动过的主题，先去 `git log`
看这份 `references/wechat-html.md` 是不是刚被人改过。**
...
报 **87** 条不达标、全部已冻结进 `references/contrast-baseline.tsv`，exit 0。
...
### 2026-08-11 下半场：对比度 116 条查出 6 条是判据假阳，已修正为 110（Task 1–3 全部收口）
...
第 7 行发现仍成立但底与比值都记错——修正后的键是 `21-aurora-flow span #38c6d9 #ffffff`，
真实对比度 2.05:1，装饰阈值 3.0。那个看着最吓人的 **`1.00:1 / 55 处` 不是「文字消失」**，
是判据把一条装饰线当成了整张卡的底。**所以真实存量是 110 条，不是 116。**

- **判据设计**：`docs/superpowers/specs/2026-08-11-background-size-backdrop-design.md`（已入仓）
- **实施计划**：`docs/superpowers/plans/2026-08-11-background-size-backdrop.md`（已入仓，**三个任务全部执行完毕**，Task 3 就是本节这次改写）
- **执行方式：Subagent-Driven**（每任务派新 subagent、任务间人工 review），三个任务都按此跑完
- **执行前后基线数字**：新判据 `image_reaches_text()` 落地后，`--prune` 先把 116 降到 **109**
  （6 条纯假阳 + 1 条错键的 aurora-flow 行一起被删，`--prune` 只删产物里已经不存在的存量行）；
  `--write-baseline` 再把修正后的 aurora-flow 行（键已改）加回来，落定为 **110**。
  当天收工时 `contrast-themes.py` 对真实库跑出 **110 条不达标、110 条基线、无新增、exit 0**
（**2026-08-13 做掉 B 档 7 支色后是 87**，当前值一律以第三节为准），
  第三节基线 9 与本节的数字已按此改到位

**三个任务之后的整支 review 又查出两处 Important，都已修完，值得单独记下来**——因为
它们两次证明了同一件事：**这道门的招牌那句「只可能多报，不可能藏发现」，比代码实际
做到的更硬**。

1. **判据曾经真的能藏掉发现。** `background-origin: content-box` 把
   `background-position` 的定位区从 padding box 挪到 content box，`top` 的条带于是贴在
   文字第一行顶上；`background-attachment: fixed` 把定位区挪到视口，padding 更是什么
   都证明不了。两种写法下四条必要条件**全部成立**，判据返回 `False`，一条真发现凭空
   消失。spec §七 原本把这两个属性和「左右条带」「`px`↔`%` 换算」并列成「真实库没人用，
   不做」——**那个理由只对「缺席才安全」的省略成立**，这两个恰好相反，是**出现才危险**。
2. **点名属性是错的修法。** 第一版修复只挡住上面那两个，复审随即找出同形的第三个：
   `background-position-x`/`-y` 是标准长写法、后写的赢，`background-position: top;
   background-position-y: bottom` 会让条带实际贴底，而判据只读简写、去查 `padding-top`，
   照样算出错误的 `False`。同形的还有 `padding-block*`（覆盖 `padding` 简写判出的那一侧）、
   `background` 简写（夹带没被看到的 origin/position/size）。**每核出一个补一个，是按下
   葫芦浮起瓢**——所以最终改成闭世界白名单：`image_reaches_text()` 只在元素声明的每一个
   `background-*`/`padding-*` 属性都落在 `_MODELED_BG_PADDING` 内时才继续往下判，集合外
   的一切（含未来 CSS 新增的、这里谁都没想到的）一律不看值直接 `True`。
   `padding-left`/`padding-right` 是**特意放行**的：贴上下边的条带，左右 padding 挪不动
   文字与条带的相对位置，不放行等于把这道门锁死在所有带悬挂缩进的元素上。
   **验证这条白名单没有收得过窄，靠的就是基线那个数**：门若失效，aurora-flow 那 7 行
   会回来。当时是 110（失效则变 116）；2026-08-13 做完 B 档 7 支色后是 **87**（失效则
   变 94）。改这道门之后**必须重跑这一条**，不要只看单测绿。

**留给下一个人的两条小工程债**（都不阻塞，见第六节第 2 条的四件事清单第 4 条与下面这条）：
`test-contrast-lib.py` 的断言真实条数是 **98**，数它要用 `grep -c '^ok   '`——
用 `grep -cE '^(ok|FAIL)'` 会多算结尾那行汇总 `ok：0 条失败`（全角冒号前 `\b` 成立），
用源码里的 `grep -c '^ok('` 又会漏掉缩在 try/except 里的四条。本轮两个 agent 各被这两种
口径骗过一次。

**同一轮做的分档清单（纯读取，零文件改动）**（⚠️ **这是 2026-08-11 那天的快照**：B 档
13 支色里 7 支已于 2026-08-13 做掉、基线降到 87，那一节还纠正了这张表的 4 个档位，
读完这张表接着往下看）：110 条真实存量落在 **42 支色**上——
...
† **这五个处数、以及三档的合计处数，都比今天重建后的语料高**（309/309/280/274/273 各标
一个 †，三档合计 1134/1568/818 同样标 †），原因见下方「语料事故」——重建后的文章比原文
少了一处 `<strong>`，受影响的不止表里挑出来当例子的这五支色，一共是**六支色**：第六支是
`23-terracotta-sun #c2593b`，落在 C 档（3 行、含一处受影响的 `strong`），没有被选进表里
当例子，但同样少了 1 处。**不要去改这几个历史数字**，它们如实记的是当时那份语料的读数；
今天用当前语料按同一套分档逻辑（每支色取它名下最大的差距决定归哪一档）重新跑一遍
`contrast-themes.py` 的 `collect()`，三档合计是 **B 1133 处、C 1565 处、D 816 处**——
比表里的历史合计各少 1、3、2，正是六支色里各自受影响的那几行叠加起来的数目。下次真要动
这些色时，用 `census-themes.py --counts <主题名>` 或直接重跑对比度普查现场重量一遍，
不要拿这张表的历史数字当准数。
...
### 2026-08-13：B 档做掉 7 支色，基线 110 → 87；同时量出「装饰签名撞车」

**做了什么。**B 档 13 支色里，7 支判定为干净可执行（全是文字行、不含装饰口径争议），
按「HSL 保色相保饱和、只压明度到刚过阈值 + 一点余量（文字取 4.6、不卡在 4.50 上）」执行：

| 主题 | 旧 → 新 | 挂在哪 | 销掉 | 压完 |
|---|---|---|---|---|
| 15-mint-breeze | `#1e7a5c` → `#1d775a` | h2 / strong / inline_code / th / keyword | 4 行 207 处 | 4.45 → 4.63 |
| 08-morandi-fog | `#506a77` → `#4e6774` | h3 / em / string / key | 2 行 78 处 | 4.41 → 4.60 |
| 23-terracotta-sun | `#7d6250` → `#785e4d` | 引用块文字 | 1 行 23 处 | 4.34 → 4.62 |
| 09-gilded-ink | `#8f6f2f` → `#87682c` | strong / inline_code | 3 行 200 处 | 4.16 → 4.60 |
| 19-candy-pop | `#4a7a9b` → `#457190` | em 高亮 | 1 行 1 处 | 4.07 → 4.60 |
| 03-spring-fresh | `#4a8058` → `#447550` | h2 文字 / h3 / strong / inline_code / th / string | 10 行 308 处 | 4.00 → 4.62 |
| 24-botanic-press | `#8a6d4e` → `#7e6447` | em / string / key | 2 行 78 处 | 4.00 → 4.61 |

七支色的 `.md` 都按**规则 11** 补了「这个明度是被对比度钉住的、不要往浅里调」的注记，
并写明压之前的比值差多少。**gilded-ink 与 morandi-fog 原有的注记里带着旧比值，已就地
重算**——不改的话机械替换会把它们变成自相矛盾的规范（第五节第 15 条 velvet-stage 那个形态）。

**验证（九条基线全绿）**：基线 diff **只减 23 行、增 0 行**，且 23 行全部落在这 7 支色上；
产物里旧色 0 处、新色 316/92/252/287/1/5/78 处；`audit-themes.py` 0 条、`census-themes.py`
0 未销 + 8 豁免、`contrast-themes.py` 87 条 / 87 基线 / 无新增 / exit 0。
⚠️ **`test-md2html.sh` 的 PART B 这一轮对这 7 份是同义反复**（先按纪律重生成了产物再跑），
和语料事故那次同理——它这轮证明不了「产物没变」，真正的证据是上面那三组数。

**中途踩到一个坑，值得记**：给 spring-fresh 写注记时引用了卡片底 `#f3f7f4`，`census-themes.py`
立刻报 `UNCARRIED`——那个 hex 是**白底 + 淡绿点纹合成出来的计算色**，theme.json 里根本没有
声明。**注记里引用底色前先确认它是声明色还是计算色**，改成散文描述即可（普查报得对）。

**没做、以及为什么。**B 档剩下 6 支：

- `27-retro-phosphor #5a7a58`：主题明文指定的三级绿，挂第六节第 3 条真机待定，不按数字改
- `23-terracotta-sun #fdf8f1`：它是**卡片底**兼 h2 白字，改法是压 h2 的底 `#c2593b`（C 档），
  两者是同一个决定；换纯白也只有 4.39，不够
- **4 支的「B」是假的**，见下——它们的档位是被判据缺陷抬上来的

**同一轮量出的判据缺陷（已记录、未修，用户裁定改判据另开一轮）**：
`contrast_lib.is_decor()` 认装饰的两把尺子之一是**样式串归一化后逐字相等**，这会撞上正文。
arena-charge 的 `strong` 是 `color: #e03131; font-weight: 800`，与它 `h3_prefix_html` 注入的
span 样式一字不差，于是**全篇 61 处正文 strong 按图形 3:1 判**。全库共 **12 行 / 123 处**
正文被这样降格（arena-charge 的 strong 与行内 code、retro-phosphor 与 lavender-dusk 的代码
token、四个主题的 em）；按文字口径重判，基线会从 87 涨到 99。**方向是藏发现**，判据教训与
修法方向已写进 `docs/theme-design-lessons.md`「机械审计方法」节。

它对分档的污染，**下一个接手 C/D 的人必须先看**：

| 色 | 表上是 | 实际 |
|---|---|---|
| `15-mint-breeze #2fa47e` | B，差 0.29 | 那 77 处「装饰」**全是代码块 token**，真装饰 0 处 → 按 4.5 判差 1.79，**D 档** |
| `25-washi-spring #8a9b6e` | B，差 0.48 | 7 处真装饰 + 77 处代码 token + 1 处 em → 按 4.5 判差 1.98，**D 档** |
| `01-autumn-warm #d97758` | B，差 0.13 | 20 处是真装饰（▶/•，3.0 合理），但夹着 **1 处 em 正文**（2.87，实需 4.5） |
| `03-spring-fresh #6b9b7a` | B，差 0.26 | 同形，em 2.74 |

**照 3.0 把后两支压到刚过线，会把一条真发现从基线里销掉而读者依旧读不清**——与 mint-breeze
那条「用假理由灭真发现」的豁免注记同族，所以这四支一并押后。

**分档和候选值怎么现场重量**（表里的数字是快照，别当准数，二十来行脚本重写比翻回来快）：
分档＝导入 `contrast-themes.py` 的 `collect()` 拿到全部发现行，每行算
`差距 = contrast_lib.threshold(kind=='装饰', size, weight) − ratio`，**按「哪支色」聚合、
取该色名下最大的那个差距定档**（B ≤0.5 / C ≤1.25 / D >1.25）——决策单元是色不是行，
改一支色常常一次销掉七八行。候选值＝把该色转 HSL、**保色相保饱和只挪明度**，逐步走到它名下
每一行都过「阈值 + 余量」为止；余量取 4.6 / 3.1，别卡在 4.50 上。挂载点用「在 `theme.json`
里搜这个 hex」列出来，顺带能看出它有没有被当成背景用（那种不能靠压明度修，`#fdf8f1` 就是）。

**顺带修的一件事**（单独一个提交）：自检脚本里那条新检查的 strip bug，见第三节第 4 条。

**下一步**：C 档 14 支 / D 档 15 支，每支都要用户单独拍板（第 2 条那句「都要动配色、都含
审美判断」仍然成立）；判据撞车那道口子是独立一轮，先补变异测试再改识别方式。

**语料事故：`wechat_test/` 今天整个消失又重建过，接手前必须知道**——
`~/code/skills/writing/wechat_test/` 目录连同 `litellm-multi-provider-gateway.md` 与
26 份定稿 HTML **全部消失**（原因不明，发现时已经没了），第三节九条基线里有四条
（第 3、4、6、9 条：第 3 条的 PART B、第 4 条的产物自检没有产物可跑、第 6 条的 L2 层、
第 9 条的 `test-contrast-themes.sh` + `contrast-themes.py`——`test-contrast-cli.py` 只测
纯函数不读语料，未受影响）因此一度 SKIP。已重建：目录重新建立，文章取自归档副本
`~/code/skills/writing/ran-publish/wechat/0001-litellm-multi-provider-gateway/litellm-multi-provider-gateway.md`，
26 份 HTML 用 `test-md2html.sh` 的权威 `PAIRS` 表，从仓库自己的 26 份 `theme.json` 重新
生成。两个后果记在这里，免得下一个人误读：

- **`test-md2html.sh` 的 PART B 这一轮是同义反复**：它的机制是拿仓库的 `theme.json` 现场
  重跑一次 `md2html.py`，把这次输出与语料目录 `out/` 里的「定稿」HTML 逐字节比对——**两侧是
  同一份代码在同一组输入上的两次独立运行**，不是一次运行产出的两份拷贝；只是这一轮语料目录
  里的「定稿」HTML 正是今天重建时刚生成的，两次运行用的是同一批重建后输入。26/26 绿这一轮
  **证明不了「产物没变」**，因为没有一份独立于今天重建之外的旧基线可比——这个比对仍能抓到
  两次运行之间的不确定性，但抓不出相对某次编辑「有没有变」。**这条护栏从下一次改动起才重新
  有牙齿**，不要把这一轮的绿读成「产物逐字节不变」的证据
- **重建后的文章比原文只差一处**：一个 `<strong>` 不见了。（判据修正前的）116 条基线键
  全部对上了没有走样，但参考处数普遍挪了 1——七个 `strong` 行各少 1 处（六个主题看得见，
  另一个是 aurora-flow
  那条本来就要被判据删掉的行，删不删都一样）、aurora-flow 的 `p #33344a` 行则多了 1 处——
  这正是一处 `<strong>` 消失、变回普通段落文字的签名。上面「分档清单」表里带 † 的**八个**
  数字（五个例子行 + 三档合计）就是这处偏移，**属于语料层面的预期抖动，不是新发现的缺陷，
  历史数字不用回改**
...
- **一条线索，现已被机械覆盖（2026-08-10 已核，结论见第六节第 2 条）**：
  ...
  `references/contrast-baseline.tsv` 现收了 **87** 条不达标组合（首次落地时是 116，
  2026-08-11 修正判据后降到 110，见第四节），这四处只是其中几条。
...
**首次落地时真实基数是 116 条，不是立项那一轮一次性探针估的 113**（116 后来又在
2026-08-11 发现 6 条判据假阳，修正为 110，见下文更正与第四节；下面这段记的是首次落地
当天的原始数字，不是当前值）。探针的装饰判据（「文本里没有字母/数字/CJK
...
**这是一道冻结基线护栏，不是完工声明**：116 条存量（首次落地时的数字，2026-08-11 已修正为
110，见下文）全部列在 `.tsv` 里但不阻塞，`exit 1`
...
`--write-baseline` 现在会在写之前**打印「相对旧基线新增 N 条组合」并把新增行逐条列出**
（首次还没有基线文件时会明说是首次、不报假的新增数）。加这个是因为它原先什么都不说，
而它重写的是整份文件（重新排序、参考列的比值与计数逐行刷新），diff 又大又吵，
几行真的新增很容易被埋在预期内的抖动里——那等于把上面那条「唯一护栏」削掉大半。
**注意这只是让增长看得见，并不拦它**：人读 diff 仍然是真正的护栏。

⚠️ **2026-08-11 更正（已收口）：这 116 条里有 6 条是判据假阳、1 条底记错，真实存量 110。**
详见第四节「2026-08-11 下半场」——`contrast_lib.py` 原先不认 `background-size`，把
aurora-flow 卡顶 4px 装饰条当成了整张卡的底。修它的 spec 与 plan **已入仓且三个任务全部
执行完毕**：新判据 `image_reaches_text()` 已入 `contrast_lib.py`，基线已从 116 经
`--prune` 降到 109、再经 `--write-baseline` 落定为 110。**下面这一段记的是首次落地当天
的数字，读它时把 116 换成 110**；处置分档（B 擦边 35 行 / C 中等 45 行 / D 重定色 30 行）
也在第四节那一条里。

**处置这 110 条是另一轮的事，性质与普查那 43 条完全不同**（**2026-08-13 已做掉其中 23 条 /
7 支色，余 87 条**，见第四节那一节）：43 条大半靠改标签、改描述、写
豁免注记就能销，产物逐字节不变；这 110 条几乎每一条都要动配色、都会改产物、都含审美判断，
...
**最终全分支评审 + 一次修复波已走完**，评审判「可合并」，无 Critical。修复波一次改掉四条
（详细数字与反例见 `docs/theme-design-lessons.md`「机械审计方法」节的第 5 条）：

- **渐变插值的数学错误**：原先是「先把每个色标合成到底上、再在两个合成结果之间插值」，
  只有两个色标 alpha 相同时才等价于真实渲染。改成**先按未预乘的 `(r,g,b,a)` 逐分量插值、
  再逐样本合成**。反例已入 lessons。**这次修完基线一个字没动**（真实库那 6 条渐变要么
  alpha 恒定、要么是硬停、要么 alpha 小到不影响判定），修完仍是 116 条 / 116 条基线 / 无新增
- **硬停判定只认 `%` 位置**：真实库的纹理渐变用的是 `px`，一直走的是 101 点插值而不是
  两点硬停。改成同单位才算同位置，**没有做 `px`↔`%` 换算**——那种等价是猜的
- `--write-baseline` 补了增长信号（见上）
- `decor_signatures` 遇到非字符串字段值改成往 stderr 打 `WARN` 再继续（不抛：一个坏字段
  不该中断全库）

**五件如实记录、不隐瞒的事**（第 1、3 条已于 2026-08-11 收口，见第四节
「2026-08-11 做完的」；第 2 条仍成立；第 4 条是那一轮新记的一条小工程债；
第 5 条是 2026-08-13 新记的）：
...
2. **`contrast-ok` 豁免注记的解析故意没做**——存量里（当时 110 条，现 87 条）没有一条已判定「可以永远这样」，
...
4. **`write_baseline` 的排序只用了键的四列，不是全部七列**——`contrast-themes.py` 里
   `sorted(findings, key=lambda f: (f.theme, f.tag, f.fg, f.bg))`，七列键里的字号、字重、
   类（装饰/文字）不参与排序，同这四列的行靠 `findings` 的迭代顺序决定先后。**对当前这份
   语料是确定性的、没有现实影响**，但下一次语料变化如果让原本不重复的四元组出现新的并列，
   排序就可能改变这些行的相对位置，制造一次与内容无关、纯粹因为排序抖动的大 diff——而
   「只许减、不许增」这条纪律唯一的护栏就是人读那份 diff，一次无意义的大 diff 会让这道
   护栏形同虚设。**这条债很小**：把 key 换成七列全排即可，留给下一次真的动这个脚本的人。
5. **第三节第 4 条那个自检报 FAIL 时退出码是 0**——`print('FAIL:…')` 之后没有
   `sys.exit(1)`，所以任何「跑一遍自检、看退出码」的脚本化用法都会把 FAIL 读成通过。
   2026-08-13 修那条 `_key()` 的 strip bug 时看见了，**没顺手改**：它不在那次改动的范围里，
   而且这份自检脚本正被另一条工作线同时改（见第三节第 4 条），两边同时动同一段代码更容易
   撞车。要改就单独改，并且把「读输出还是读退出码」这件事在 `references/wechat-html.md`
   里写清楚。
...
- **`md2publish-cover` 的端到端付费 smoke 未跑**：本机一个 provider 凭证都没配，
  真调一次 provider 生一张图并压到 2MB 内这一步从未做过。它的十二项机械检查全绿
  （`check.sh` 三期后由九项扩到十二项），
  但**十二项绿不等于端到端验证过**。
...
- `superpowers:test-driven-development` — 给 `md2html.py` 加字段时照搬 `test-md2html.sh` 的范式：先造会失败的用例，再实现。**改 `is_decor()` 那道撞车口子时也走这条**：先往 `test-contrast-themes.sh` 里加会红的变异，再改识别方式
- `superpowers:brainstorming` + `AskUserQuestion` — 处置 C/D 两档时逐支色跟用户对齐再动手；本轮 B 档就是先出「候选值 + 挂载点 + 影响面」的表、拿到裁决才改文件，中途还因此拦下了 4 支档位不实的色
- `tech-writer` — 校订教程文档时
