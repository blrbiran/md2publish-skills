...
第 3 条的 PART B 从 `references/theme-json/` 读 theme.json、与实验目录里的定稿 HTML 逐字节比对。**故意改了某份 theme.json 之后要先重新生成它的 HTML 再跑**，否则那里报的红是预期内的改动，不是回归——别反过来改测试迁就它。语料目录缺失时 PART B 会整体 SKIP 并把退出码标红（静默跳过等于没有护栏）。⚠️ **2026-08-11 这一轮的 26/26 绿不算回归证据**：语料目录当天整个消失又重建过，PART B 比对的两侧是同一次 `md2html.py` 运行产出的两份拷贝，同义反复，从下一次真正改动起才重新有牙齿——详见第四节「2026-08-11 下半场」的语料事故记录。
...
† **这五个处数比今天重建后的语料高 1**（309/309/280/274/273 各标一个 †），原因见下方
「语料事故」——重建后的文章比原文少了一处 `<strong>`，这几支色各少了一次命中。**不要去改
这几个历史数字**，它们如实记的是当时那份语料的读数；下次真要动这些色时，用
`census-themes.py --counts <主题名>` 现场重量一遍，不要拿这张表当准数。
...
- **`test-md2html.sh` 的 PART B 这一轮是同义反复**：它拿 md2html 的输出去比对语料目录里的
  「定稿」HTML，而这一轮语料目录里的 HTML 正是 md2html 刚生成的——26/26 绿这一轮**证明不了
  「产物没变」**，因为比较双方是同一次运行的两份拷贝。**这条护栏从下一次改动起才重新有牙齿**，
  不要把这一轮的绿读成「产物逐字节不变」的证据
- **重建后的文章比原文只差一处**：一个 `<strong>` 不见了。（判据修正前的）116 条基线键
  ...
  这正是一处 `<strong>` 消失、变回普通段落文字的签名。上面「分档清单」表里带 † 的五个数字
  就是这处偏移，**属于语料层面的预期抖动，不是新发现的缺陷，历史数字不用回改**
