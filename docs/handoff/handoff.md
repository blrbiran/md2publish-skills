...
> 更新于 2026-08-10（第八轮：对比度护栏落地——脚本、单测、变异测试、116 条冻结基线全部入仓）。仓库位置：`~/code/skills/writing/md2publish-skills/`
>
...
9. **对比度护栏本轮已落地**（第六节第 2 条）：第四套机械脚本 `contrast-themes.py` + 冻结基线
   `references/contrast-baseline.tsv` 已入仓。真实基数是 **116 条**（第七轮探针估的 113 只是
   下限，用的装饰判据已被 spec 否掉，别再当事实抄）。**存量不阻塞，冻结基线只挡新增组合**，
...
**真实基数是 116 条，不是第七轮探针估的 113。**探针的装饰判据（「文本里没有字母/数字/CJK
