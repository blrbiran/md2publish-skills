...
- 引用文字：`#61638a`（在浅紫底 `#ecebfb` 上 4.88:1）
- 代码块底：`#282a45`（深紫蓝）、代码默认字：`#e8e8f8`
- 代码块内注释：`#9a9cc0`、关键字：`#9a8fff`（两者在代码底上分别 5.23:1 / 5.14:1）
- 浅紫底：`#ecebfb`
- 边线：`#e3e3f0`
...
- 曾经声明过一个「次级灰」并要求「必须真的用上」，实测产物里 **0 处**：它派的四个落点里，引用块文字被 `#61638a` 占了、代码注释被另一个色占了、「题注」和「次要说明段」机械层根本没有对应的块类型。**指不出可达落点的颜色就从调色板里删掉**（规则 3），不要靠加一句「必须真的用上」把它留着——那句话不会让它出现在产物里
...
- 代码块语法高亮（**取色只在本主题调色板内选，够不到 4.5:1 就退回默认字色，不要现造新色**）：注释 `#9a9cc0`、关键字 `#9a8fff`、字符串与键名 `#38c6d9`、数值退回默认字 `#e8e8f8`。
  极光紫 `#6a5cff` 在这个深底上只有 3.04:1，**不能拿来做代码里的文字色**——它是浅底上的强调色，换到深底就不成立了
