...
- 深金（strong 用，更沉）：`#87682c`
- 次级灰褐：`#6b6355`
- 金边线：`#e5ddc9`
- 引用/代码底：`#f5f1e8`
- 代码块内关键字/键名：`#7a5b1f`、字符串：`#6a4f1a`（在代码底 `#f5f1e8` 上分别 5.57:1 / 6.78:1）

> 这两档金的明度是被对比度铁律钉住的，**不要往浅里调回调色板上面那两支金**：在代码底 `#f5f1e8` 上，深金 `#87682c` 是 4.60:1（刚过 AA，2026-08 为此从更浅的一档压深过一次，别调回去），古金 `#b08a3e` 只有 2.84:1、不达 AA，所以它只做线和符号（规则 11）。也不要图省事退回代码默认字 `#3a352c`（10.80:1）——那会让代码块整块塌成单色，「鎏金」的识别点恰恰在代码里那点金。同一支金在白卡上和在代码底上是两个东西（规则 12）。
...
- strong：`color: #87682c; font-weight: 700`
- em：`color: #6b6355`
- 关键数字（财经文常见）可用 `color: #87682c; font-weight: 700; font-size: 17px` 微放大
...
- 代码块：`<pre>` 底 `#f5f1e8`、文字 `#3a352c`、`border: 1px solid #e5ddc9; border-radius: 6px; padding: 15px 17px; font-size: 13px`；行内 code：底 `#f5f1e8`、文字 `#87682c`
